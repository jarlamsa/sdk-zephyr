			    ...
			    bool last_block, size_t total_size)
{
	...
	if (state == SW_MGMT_UPDATE_STATE_INITIAL) {
		...
		if (data_len == 0U && state == SW_MGMT_UPDATE_STATE_DOWNLOADED) {
			...
			lwm2m_software_mgmt_set_update_result(obj_inst_id, SW_MGMT_UPDATE_RESULT_DEFAULT);
			return 0;
		}
	...
	if (ret >= 0) {
		if (last_block) {
			lwm2m_software_mgmt_set_update_state(obj_inst_id, SW_MGMT_UPDATE_STATE_DOWNLOADED);
			lwm2m_software_mgmt_set_update_state(obj_inst_id, SW_MGMT_UPDATE_STATE_DELIVERED);
		}

		return 0;
	} else if (ret == -ENOMEM) {
		lwm2m_software_mgmt_set_update_result(obj_inst_id, SW_MGMT_UPDATE_RESULT_OUT_OF_MEM);
	} else if (ret == -ENOSPC) {
		lwm2m_software_mgmt_set_update_result(obj_inst_id, SW_MGMT_UPDATE_RESULT_OUT_OF_STORAGE);
		/* Response 4.13 (RFC7959, section 2.9.3) */
		/* TODO: should include size1 option to indicate max size */
		ret = -EFBIG;
	} else if (ret == -EFAULT) {
		lwm2m_software_mgmt_set_update_result(obj_inst_id, SW_MGMT_UPDATE_RESULT_INTEGRITY_CHECK_FAILED);
	} else {
		lwm2m_software_mgmt_set_update_result(obj_inst_id, SW_MGMT_UPDATE_RESULT_UPDATE_ERROR);
	}
				  ...
				  uint8_t *args, uint16_t args_len)
{
	...
	if (callback) {
		...
		lwm2m_software_mgmt_set_activation_state(obj_inst_id, SW_MGMT_ACTIVATION_STATE_ENABLED);
	}
				  ...
				  uint8_t *args, uint16_t args_len)
{
	...
	if (callback) {
		...
		lwm2m_software_mgmt_set_activation_state(obj_inst_id, SW_MGMT_ACTIVATION_STATE_DISABLED);
	}
...
static struct lwm2m_engine_obj_inst *software_mgmt_create(uint16_t obj_inst_id)
{
	...
	INIT_OBJ_RES_EXECUTE(SOFTWARE_MGMT_DEACTIVATE_ID, res[index], i, software_mgmt_deactivate_cb);
